...
internal class ModsScreen : IScreen
{
    ...
    {
        get => selectedIndex == -1 ? QuintessentialLoader.QuintessentialModMeta : QuintessentialLoader.Mods[class_162.method_404(selectedIndex, 0, QuintessentialLoader.Mods.Count - 1)];
    }
    ...
    public void method_50(float time)
    {
        ...
        if (Input.IsSdlKeyHeld(SDL.enum_160.SDLK_DOWN))
        {
            selectedIndex = Math.Min(selectedIndex + 1, QuintessentialLoader.Mods.Count - 1);
        }
        else if (Input.IsSdlKeyHeld(SDL.enum_160.SDLK_UP))
        {
