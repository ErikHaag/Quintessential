...
public class PuzzleModel
{
    ...
    public static Puzzle FromModel(PuzzleModel model)
    {
        ...
        if (model.Payloads != null)
        {
            ...
            foreach (Payloads.Payload payload in ((patch_Puzzle)(object)ret).Payloads.method_1087().PuzzleInitialization)
            {
                foreach (Pair<string, PuzzlePayloadHandler> handler in QApi.PuzzlePayloadHandlers)
                {
                    if (handler.Left == payload.Address)
                    {
                        handler.Right(ret, payload.Data);
                    }
                }
            }
        }
